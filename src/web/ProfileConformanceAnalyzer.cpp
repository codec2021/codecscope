...
namespace web
{
  ...
  {
    return pSPS -> log2_min_luma_coding_block_size_minus3 + 3 + pSPS -> log2_diff_max_min_luma_coding_block_size;
  }
  ...
  std::vector<uint32_t> ProfileConformanceAnalyzer::columnWidthInLumaSamples(std::shared_ptr<HEVC::PPS> pPPS)
  {
    ...
    uint32_t CtbLog2SizeY = ctbLog2SizeY(pSPS);
  ...
  void ProfileConformanceAnalyzer::checkMainStillPictureSPS(std::shared_ptr<HEVC::SPS> pSPS, const HEVC::Parser::Info *pInfo)
  {
    ...
    if(ctbLog2SizeY(pSPS) < 4 || ctbLog2SizeY(pSPS) > 6)
      sendWarning("Main Still Picture profile: CtbLog2SizeY shall be in the range of 4 to 6, inclusive.", pInfo);
  }
  ...
  void ProfileConformanceAnalyzer::onWarning(const std::string &, const HEVC::Parser::Info *, HEVC::Parser::WarningType)
  {
  }
